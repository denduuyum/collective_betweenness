...
def arg_setup():
    ...
    parser.add_argument('-n', '--name', dest='name', type = str, required = False,
                    help='test name')
    parser.add_argument('-d', '--depth', dest='depth', type = int, help='Depth level', default = 1)
...
def main():
    ...
    with open('./output/' + args.file, 'w') as fout:
        fout.write(str(F10) + '\n')
        fout.write(str(sus) + '\n')
    F = F10
    ...
    print('-------------------------------------')
    cbc_sum = collective_bc_sum(G, bc, args.depth)
